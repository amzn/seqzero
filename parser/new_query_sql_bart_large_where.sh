...
for f in $TASK/bart-checkpoints$PARAM/checkpoint99*
do 
    echo '------------------------------------------------------------'
    echo $f
    ...
    python normalize_and_generate_prompt.py -in_file $TASK/pred$PARAM.test -out_file data/geo_sql_query_group/prompt_seq.test -reg_term "where" -in_prompt ", the setence requires" -out_prompt " , the setence requires to group by"
done
