TASK=data/geo_sql_query_select
PARAM='-large'
...
for f in $TASK/bart-checkpoints$PARAM/checkpoint23*
do 
    echo '------------------------------------------------------------'
    echo $f
    ...
    python normalize_and_generate_prompt.py -in_file $TASK/pred$PARAM.test -out_file data/geo_sql_query_where/prompt_seq.test -reg_term "select" -in_prompt ", the sentence asks to select" -out_prompt " , the setence requires"
done
