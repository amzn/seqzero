...
do 
    ...
    do
        ...
        echo $f
        echo $lambda
        CUDA_VISIBLE_DEVICES=$1 python semantic_parsing.py --model-dir $(dirname $f) --model-file "$(basename $f):model.pt" --src $TASK/src.test --out $TASK/pred$PARAM.test --data-dir "${TASK}/bin-large-prompt/" --use-prefix-constriant --prompt "$outprompt" --prefix-file prompt_prefixes.txt --weight-ensemble=$lambda
        python acc_sql.py -pred $TASK/pred$PARAM.test -gold $TASK/tgt_prompt.test
    done
done
