...
def load_model_ensemble_and_task(
    ...
    if "weight_emsemble" in arg_overrides:
        weight_emsemble = arg_overrides["weight_emsemble"]
    if weight_emsemble>0:
        org_esemble = ensemble
        assert(len(org_esemble)==2)
        theta_0 = org_esemble[0].state_dict()
        theta_1 = org_esemble[1].state_dict()
        assert set(theta_0.keys()) == set(theta_1.keys())
        theta = {key: (1-weight_emsemble) * theta_0[key] + weight_emsemble * theta_1[key] for key in theta_0.keys()}
        org_esemble[0].load_state_dict(theta)
        ensemble = [org_esemble[0]]
