...
class BARTHubInterface(GeneratorHubInterface):
    ...
    def generate(
        ...
        **kwargs
        ...
        res = []
        if prefix:
            prefix_tokens = self.bpe.encode(prefix.strip())
            if len(prefix_tokens.split(" ")) > min(self.max_positions) - 2:
                prefix_tokens = " ".join(prefix_tokens.split(" ")[: min(self.max_positions) - 2])
            prefix_tokens = self.task.source_dictionary.encode_line(prefix_tokens, append_eos=False)
        for batch in self._build_batches(tokenized_sentences, skip_invalid_size_inputs, constraints_tensor):
            ...
            if prefix:
                prefix_tokens = prefix_tokens.view(1, -1).repeat(src_tokens.size(0), 1).to(device=self.device)
                inference_step_args["prefix_tokens"] = torch.cat((inference_step_args["prefix_tokens"], prefix_tokens), 1)
            elif diff_prompts:
                assert not prefix
                final_prefixes = []
                for id in batch['id'].tolist():
                    prefix_tokens = self.bpe.encode(diff_prompts[id].strip())
                    if len(prefix_tokens.split(" ")) > min(self.max_positions) - 2:
                        prefix_tokens = " ".join(prefix_tokens.split(" ")[: min(self.max_positions) - 2])
            
                    final_prefixes.append(self.task.source_dictionary.encode_line(prefix_tokens, append_eos=False))
                prefix_tokens = torch.nn.utils.rnn.pad_sequence(final_prefixes, batch_first=True, padding_value=self.task.target_dictionary.pad()).to(device=self.device)
                inference_step_args["prefix_tokens"] = torch.cat((inference_step_args["prefix_tokens"], prefix_tokens), 1)
            if constraints_tensor is not None:
                assert("constraints" in batch)
            ...
            results = super().generate(
                ...
                *args,
                ...
                constraints_tensor=constraints,
                **kwargs
            )
