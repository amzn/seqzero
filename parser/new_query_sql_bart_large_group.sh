...
for f in $TASK/bart-checkpoints$PARAM/checkpoint10.pt
do 
    echo '------------------------------------------------------------'
    echo $f
    ...
    python normalize_and_generate_prompt.py -in_file $TASK/pred$PARAM.test -out_file data/geo_sql_query_order/prompt_seq.test -reg_term "group by" -in_prompt ", the setence requires to group by" -out_prompt " , the setence requires the result to be ordered by"
done
